...
use stratum_common::{
    network_helpers_sv2::{
        ...
        noise_iroh_stream::{NoiseIrohReadHalf, NoiseIrohWriteHalf},
    },
    roles_logic_sv2::{
        bitcoin::{self, TxOut},
...
/// Enum to wrap both TCP and Iroh read halves
pub enum NoiseReader {
    Tcp(NoiseTcpReadHalf<Message>),
    Iroh(NoiseIrohReadHalf<Message>),
}

impl NoiseReader {
    pub async fn read_frame(&mut self) -> Result<EitherFrame, stratum_common::network_helpers_sv2::Error> {
        match self {
            NoiseReader::Tcp(reader) => reader.read_frame().await,
            NoiseReader::Iroh(reader) => reader.read_frame().await,
        }
    }
}

/// Enum to wrap both TCP and Iroh write halves
pub enum NoiseWriter {
    Tcp(NoiseTcpWriteHalf<Message>),
    Iroh(NoiseIrohWriteHalf<Message>),
}

impl NoiseWriter {
    pub async fn write_frame(&mut self, frame: EitherFrame) -> Result<(), stratum_common::network_helpers_sv2::Error> {
        match self {
            NoiseWriter::Tcp(writer) => writer.write_frame(frame).await,
            NoiseWriter::Iroh(writer) => writer.write_frame(frame).await,
        }
    }
}
...
/// Spawns async reader and writer tasks for Noise-wrapped streams (TCP or Iroh).
#[track_caller]
#[allow(clippy::too_many_arguments)]
pub fn spawn_io_tasks_with_noise(
    task_manager: Arc<TaskManager>,
    mut reader: NoiseReader,
    mut writer: NoiseWriter,
    outbound_rx: Receiver<SV2Frame>,
    inbound_tx: Sender<SV2Frame>,
    notify_shutdown: broadcast::Sender<ShutdownMessage>,
    status_sender: StatusSender,
) {
    // Delegate to the generic implementation based on the variant
    // Both TCP and Iroh now have the same read_frame/write_frame API through the enum
    ...
    let outbound_rx_clone = outbound_rx.clone();
    {
        ...
        task_manager.spawn(async move {
            trace!("Reader task started (Noise wrapper)");
            loop {
                tokio::select! {
            ...
            warn!("Reader task exited (Noise wrapper).");
...

    {
        ...
        task_manager.spawn(async move {
            trace!("Writer task started (Noise wrapper)");
            loop {
                tokio::select! {
                    message = shutdown_rx.recv() => {
                        match message {
                            ...
                            Ok(ShutdownMessage::DownstreamShutdownAll) => {
                                trace!("Received downstream shutdown all");
                                if status_type != StatusType::TemplateReceiver {
                                    outbound_rx.close();
                                    break;
                                }
            ...
            warn!("Writer task exited (Noise wrapper).");
        }.instrument(tracing::info_span!(
            "writer_task_noise",
            spawned_at = %format!("{}:{}", caller.file(), caller.line())
        )));
